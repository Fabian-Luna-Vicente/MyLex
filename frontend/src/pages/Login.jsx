...
export default function Login() {
  ...
  const handleFakeGoogleLogin = async () => {
    // Para propósitos de desarrollo sin configurar el script de Google:
    // En producción usarías @react-oauth/google
    setLoading(true);
    setError('');
    // Simula un token de google. Reemplazar luego con la respuesta real de Google.
    ...
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-slate-800 p-8 rounded-2xl border border-slate-700 shadow-2xl">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 mt-2 text-sm">Sign in to sync your vocabulary across devices.</p>
        </div>
